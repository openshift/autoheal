...
type HealingRuleLister interface {
	...
	List(selector labels.Selector) (ret []*v1alpha1.HealingRule, err error)
...
// List lists all HealingRules in the indexer.
func (s *healingRuleLister) List(selector labels.Selector) (ret []*v1alpha1.HealingRule, err error) {
	err = cache.ListAll(s.indexer, selector, func(m interface{}) {
		ret = append(ret, m.(*v1alpha1.HealingRule))
	})
	return ret, err
}
...
type HealingRuleNamespaceLister interface {
	...
	Get(name string) (*v1alpha1.HealingRule, error)
...
// List lists all HealingRules in the indexer for a given namespace.
func (s healingRuleNamespaceLister) List(selector labels.Selector) (ret []*v1alpha1.HealingRule, err error) {
...
// Get retrieves the HealingRule from the indexer for a given namespace and name.
func (s healingRuleNamespaceLister) Get(name string) (*v1alpha1.HealingRule, error) {
	obj, exists, err := s.indexer.GetByKey(s.namespace + "/" + name)
	if err != nil {
		return nil, err
	}
	if !exists {
		return nil, errors.NewNotFound(v1alpha1.Resource("healingrule"), name)
	}
	return obj.(*v1alpha1.HealingRule), nil
}
